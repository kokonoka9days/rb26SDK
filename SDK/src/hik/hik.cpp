...
namespace sdk{
    ...
    bool HikCamera::CameraInit( char *sn, 
        ...
        this->nDeviceNum = device_list.nDeviceNum - 1;//这个我也不知道为啥，返回的设备数量总是比实际插入设备总数量多一个
        ...
        MV_CC_CreateHandle(&camera_handle_, device_list.pDeviceInfo[cameraIndex]);//默认打开设备列表的第一个相机
    ...
    bool HikCamera::capture_start(double exposure_us_,double gain, double gamma)
    {
        ...
        */
        ...
        nRet = MV_CC_SetFloatValue(camera_handle_, "Gain", gainRange.fMax*gain);
        if (nRet != MV_OK) {
